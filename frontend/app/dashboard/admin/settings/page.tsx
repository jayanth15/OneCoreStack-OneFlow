...
export default function SettingsPage() {
  ...
  useEffect(() => {
    apiFetchJson<UnitItem[]>("/api/v1/units?include_inactive=true")
      .then(setUnits)
      .catch(() => {})
  ...
  async function refreshUnits() {
    const data = await apiFetchJson<UnitItem[]>("/api/v1/units?include_inactive=true");
    setUnits(data);
  }
  ...
  async function checkUsage(id: number) {
    const data = await apiFetchJson<{ total: number; by_table: Record<string, number> }>(
