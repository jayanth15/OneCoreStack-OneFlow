"""Legacy database migrations — one-time catch-up for pre-Alembic databases.

This module contains all the hand-written schema migrations that were run
on startup before Alembic was introduced. It is kept solely so that existing
databases (created before Alembic) can be brought up to the baseline schema
one final time before being stamped.

Once all deployments have been stamped (alembic_version table exists),
...
def run_all() -> None:
    """Run all legacy migrations in order. Idempotent — safe to re-run."""
    ...
    logger.info("Legacy migrations complete")
    # Run database.py run_migrations() too (legacy inline migrations)
    from app.core.legacy_db_migrations import run_inline_migrations
