...
def get_linkable_pr_items(session: Session, pr_id: int) -> list[LinkablePROut]:
    ...
    inventory_ids = {item.inventory_item_id for item in items if item.inventory_item_id}
    inventories = (
        {
            inventory.id: inventory
            for inventory in session.exec(
    ...
    unit_ids = {
        inventory.unit_id for inventory in inventories.values() if inventory.unit_id
    }
    units = (
        {
            unit.id: unit
            for unit in session.exec(select(Unit).where(Unit.id.in_(unit_ids))).all()
        }
        if unit_ids
        else {}
    )
    ...
    for i, it in enumerate(items, start=1):
        inventory = inventories.get(it.inventory_item_id)
        unit_id = inventory.unit_id if inventory else None
