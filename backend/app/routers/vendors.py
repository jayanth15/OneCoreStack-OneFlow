...
Vendors are stored in their own table so new schedules *must* reference
an existing vendor.  Schedule-derived stats are merged in at list/detail time.

GET  /api/v1/vendors              — list all vendors with aggregated stats
GET  /api/v1/vendors/names        — lightweight [{id, name}] list for dropdowns
POST /api/v1/vendors              — create a new vendor (admin+)
GET  /api/v1/vendors/{name}       — full detail for one vendor
"""
...
def create_vendor(
    ...
    """
    Create a new vendor / OEM client (admin / super_admin only).
...
def list_vendors(
    ...
    """
    Returns one entry per vendor (from the Vendor table, enriched with
    schedule stats).  Vendors with no schedules yet also appear.
    """
    # All registered vendors
    registered = {c.name: c for c in session.exec(select(Vendor).order_by(Vendor.name)).all()}  # type: ignore[union-attr]
    ...
    result = []
    for vendor_name in all_names:
        if search and search.lower() not in vendor_name.lower():
            continue

        scheds = by_customer.get(vendor_name, [])
        ...
        c = registered.get(vendor_name)
        result.append({
            "vendor_name": vendor_name,
            "vendor_id": c.id if c else None,
