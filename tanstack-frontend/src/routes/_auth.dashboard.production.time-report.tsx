...
export const Route = createFileRoute("/_auth/dashboard/production/time-report")({
...
function TimeReportPage() {
  ...
  // The report fetch is triggered by this state (worker + effective date range)
  const [queryArgs, setQueryArgs] = useState<{ wId: number | "all" | null; from: string; to: string }>({
    wId: null, from: "", to: "",
  })

  function buildUrl(wId: number | "all", from?: string, to?: string) {
  ...
  const reportQuery = useQuery({
    queryKey: [buildUrl(queryArgs.wId as number | "all", queryArgs.from || undefined, queryArgs.to || undefined)],
    enabled: queryArgs.wId !== null,
    staleTime: 0,
  })

  function fetchReport(wId: number | "all" | null, from?: string, to?: string) {
    if (wId === null) return
    setQueryArgs({ wId, from: from ?? dateFrom, to: to ?? dateTo })
  }

  function handleWorkerSelect(value: string) {
    if (!value) {
      setSelectedWorkerId(null)
      setQueryArgs({ wId: null, from: "", to: "" })
      return
    }
    if (value === "all") {
...
${section("By Machine", machineRows, "<th>Machine</th><th style='text-align:right'>Hours</th><th style='text-align:right'>Qty</th><th style='text-align:right'>Cards</th>")}
  ...
  const data = (reportQuery.data as WorkerTimeSummary[] | undefined) ?? []
  const loading = reportQuery.isLoading || reportQuery.isFetching
  const error = reportQuery.error instanceof Error ? reportQuery.error.message : null
  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <Link to="/dashboard/production" className="p-1.5 rounded-md hover:bg-muted transition-colors" aria-label="Back">
