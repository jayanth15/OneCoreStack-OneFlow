...
function PlanningPage() {
  ...
  const listUrl = (() => {
    ...
    return `/api/v1/production/plans?${params}`
  })()
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) => apiFetchJson(`/api/v1/production/plans/${id}`, { method: "DELETE" }),
    onSuccess: () => {
      setDeleteId(null)
  ...
  const data       = listQuery.data as PaginatedPlans | undefined
  const loading    = listQuery.isLoading || listQuery.isFetching
  const fetchError = listQuery.error instanceof Error ? listQuery.error.message : null
  const showError  = fetchError ?? error
  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <>
            <Link to={dynTo("/dashboard/production")}
              className="p-1.5 rounded-md hover:bg-muted transition-colors" aria-label="Back">
              <ArrowLeft className="size-4" />
            </Link>
            {admin && (
              <Button size="sm" onClick={() => navigate({ href: "/dashboard/production/planning/new" })}>
        ...
        <div className="md:hidden space-y-3">
          {loading ? (
            ...
            <div className="rounded-lg border px-4 py-10 text-center text-muted-foreground text-sm">
              {searchTerm ? `No plans matching "${searchTerm}".` : `No plans found. Click "New Plan" to get started.`}
            </div>
          ) : plans.map((plan) => (
              ...
              {admin && (
                <div className="flex justify-end gap-1 pt-1 border-t">
                  <Button variant="ghost" size="icon" className="size-8"
                    onClick={() => navigate({ href: dynTo(`/dashboard/production/planning/${plan.id}/edit`) })} title="Edit">
                    ...
                    <td className="px-4 py-3 text-right">
                      {admin && (
                        <div className="inline-flex gap-1">
                          <Button variant="ghost" size="icon" className="size-8"
                            onClick={() => navigate({ href: dynTo(`/dashboard/production/planning/${plan.id}/edit`) })}
                            title="Edit">
                            <Pencil className="size-3.5" />
                          </Button>
        ...
        {!loading && totalPages > 1 && (
          <div className="flex items-center justify-between gap-2 pt-1">
            <p className="text-sm text-muted-foreground">
              Page {pageNum} of {totalPages} &mdash; {total} plan{total !== 1 ? "s" : ""}
            </p>
