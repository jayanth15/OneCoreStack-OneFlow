...
import type {UnifiedRequest} from "@/lib/requests"
...
function HistoryPage() {
  ...
  // Linked request / receipt for the open dialog
  const requestQuery = useQuery({
    queryKey: [`/api/v1/requests/${selectedHistory?.request_id}`],
    enabled: !!selectedHistory?.request_id,
    staleTime: 0,
  })
  const receiptQuery = useQuery({
    queryKey: [`/api/v1/receipts/${selectedHistory?.receipt_id}`],
    enabled: !!selectedHistory?.receipt_id,
    staleTime: 0,
  })
  ...
  const linkedRequest = requestQuery.data as UnifiedRequest | undefined
  const linkedReceipt = receiptQuery.data as Receipt | undefined

  useEffect(() => {
    if (requestQuery.isPending || receiptQuery.isPending) {
      setDetailLoading(true)
      return
    }
    if (selectedHistory) setDetailLoading(false)
  }, [requestQuery.isPending, receiptQuery.isPending, selectedHistory])
  ...
  const loadTab = useCallback(async (tab: TabKey, page: number, append: boolean) => {
    ...
    try {
      const url = `/api/v1/history/${tab}?${buildQuery(page)}`
      const data = await queryClient.fetchQuery<HistoryPage>({
        queryKey: [url],
        staleTime: 0,
      })
      setTabState(prev => ({
              ...
              <div className="rounded-lg border p-4 space-y-3">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <div className="flex gap-2">
                    {linkedRequest && <Button size="sm" variant="outline" onClick={() => navigate({ href: dynTo(`/dashboard/requests?highlight=${linkedRequest.id}`) })}>Open request</Button>}
                    {linkedReceipt && <Button size="sm" variant="outline" onClick={() => navigate({ href: dynTo(`/dashboard/receipts?receipt=${linkedReceipt.id}`) })}>Open receipt</Button>}
                  </div>
                </div>
