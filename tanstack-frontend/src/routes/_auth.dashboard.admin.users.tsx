...
function UsersPage() {
  ...
  const listUrl = `/api/v1/admin/users?include_inactive=${showInactive}`
  ...
  const users = (listQuery.data as User[] | undefined) ?? []
  const loading = listQuery.isLoading || listQuery.isFetching
  const error = listQuery.error instanceof Error ? listQuery.error.message : null

  const deleteMutation = useMutation({
    mutationFn: (id: number) => apiFetchJson(`/api/v1/admin/users/${id}`, { method: "DELETE" }),
    onSuccess: () => {
      setDeleteId(null)
      ...
      <PageHeader
        ...
        actions={
          <>
            ...
            <Button size="sm" onClick={() => navigate({ href: "/dashboard/admin/users/new" })}>
              ...
              <div key={user.id} className="rounded-lg border p-4 space-y-2.5">
                ...
                <div className="flex justify-end gap-1 pt-1 border-t">
                  <Button variant="ghost" size="icon" className="size-8"
                    onClick={() => navigate({ href: dynTo(`/dashboard/admin/users/${user.id}/edit`) })} title="Edit">
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="inline-flex gap-1">
                        <Button
                          ...
                          size="icon"
                          className="size-8"
                          onClick={() => navigate({ href: dynTo(`/dashboard/admin/users/${user.id}/edit`) })}
                          title="Edit"
                        >
                          <Pencil className="size-3.5" />
                        </Button>
