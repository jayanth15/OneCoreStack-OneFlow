...
export const Route = createFileRoute("/_auth/dashboard/inventory/cycle-count")({
...
// Primary URL per type — used as the cache key for the selected section.
function rowsUrlForType(type: string): string {
  switch (type) {
    case "consumable": return "/api/v1/consumables?page_size=500&include_inactive=false"
    case "attachment": return "/api/v1/attachments?page_size=500&include_inactive=false"
    case "weeder": return "/api/v1/weeders?page_size=500&include_inactive=false"
    case "spare": return "/api/v1/spares/categories?page_size=200&include_inactive=false"
    default: return `/api/v1/inventory?item_type=${type}&page_size=500&include_inactive=false`
  }
}
...
function CycleCountPage() {
  ...
  const chipCounts = (chipsQuery.data) ?? {}

  // Fetch rows — only the selected type is loaded, on every type change.
  const rowsUrl = rowsUrlForType(effectiveType)
  const rowsQuery = useQuery({
    queryKey: [rowsUrl],
    queryFn: () => loadRowsForType(effectiveType),
  })

  const loading = rowsQuery.isPending
  const error = loading ? null : (rowsQuery.error instanceof Error ? rowsQuery.error.message : null)
