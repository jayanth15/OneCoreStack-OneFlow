...
export const Route = createFileRoute("/_auth/dashboard/production/planning/new")({
...
function NewPlanPage() {
  ...
  const schedules = (schedulesQuery.data as PaginatedSchedules | undefined)?.items ?? []
  const schedLoading = schedulesQuery.isLoading
        ...
        <form onSubmit={handleSave} className="space-y-8">
          ...
          <div className="flex gap-3 pt-2">
            ...
            <Button type="button" variant="outline"
              onClick={() => navigate({ href: "/dashboard/production/planning" })} disabled={saving}>
              Cancel
            </Button>
          </div>
        </form>
