...
function SparesPage() {
  ...
  // When categories arrive (initial load or a new search), reset the tree and,
  // while searching, run the "search all" fan-out: pre-load every category's
  // sub-categories, every sub's matching items, and variants of variant-matched
  // items so search results are visible expanded (same Promise.all fan-out as
  // the original page — kept as plain apiFetchJson calls).
  const [fanoutLoading, setFanoutLoading] = useState(false)
  useEffect(() => {
    const data = catsQuery.data as CatsPage | undefined
    if (!data) return
    ...
    ;(async () => {
      try {
        ...
        if (!cancelled) setFanoutLoading(false)
      }
    })()
    return () => { cancelled = true }
     
  }, [catsQuery.data, search])

  // ── Data fetching ───────────────────────────────────────────────────────────

  // Refetch pages + refresh after CRUD. The initial load and search-triggered
  // reload are handled by the catsQuery above (keyed by URL); this function is
  // kept for "Load more" (append) and post-save refreshes, identical to original.
  ...
  const listError = catsQuery.error instanceof Error ? catsQuery.error.message : error
  const listLoading = catsQuery.isLoading || loading || fanoutLoading
  ...
  return (
    <>
      <PageHeader
        ...
        actions={admin ? (
          <Button size="sm" onClick={() => navigate({ href: "/dashboard/inventory/spares/new" })}>
                                    ...
                                    {loadingItems ? (
                                      ...
                                      <div className="pl-12 pr-4 py-4 text-center text-sm text-muted-foreground">
                                        ...
                                        {admin && <Button size="sm" variant="link" className="ml-2 h-auto p-0"
                                          onClick={()=>navigate({ href: `/dashboard/inventory/spares/${cat.id}/sub-categories/${sub.id}/items/new` })}>
                                          Add first item →
                                        </Button>}
                                      </div>
                                    ) : (
