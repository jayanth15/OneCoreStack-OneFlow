...
export const Route = createFileRoute("/_auth/dashboard/inventory/attachments")({
...
function displayName(item: AttachmentItem) {
  return item.sn_no || item.description || `Item #${item.id}`
}
...
function AttachmentsPage() {
  ...
  const documentsQuery = useQuery({
    queryKey: [`/api/v1/attachments/${viewItem?.id}/documents`],
    enabled: viewItem !== null,
    staleTime: 0,
  })

  const historyQuery = useQuery({
    queryKey: [
      `/api/v1/attachments/${historyItem?.id}/history?limit=10&offset=${(historyPage - 1) * 10}`,
    ],
  ...
  const saveMutation = useMutation({
    mutationFn: (body: {
      ...
      apiFetchJson(dialog === "create" ? "/api/v1/attachments" : `/api/v1/attachments/${editing!.id}`, {
  ...
  const adjustMutation = useMutation({
    mutationFn: (body: {
      ...
      apiFetchJson(`/api/v1/attachments/${adjustItem!.id}/adjust`, {
        method: "POST",
        body: JSON.stringify(body),
      }),
    onSuccess: () => {
  ...
  const docUploadMutation = useMutation({
    ...
    onError: (e: unknown) => {
      setDocumentError(e instanceof Error ? e.message : "Upload failed")
    },
  })

  const docDeleteMutation = useMutation({
    mutationFn: (documentId: number) =>
      apiFetchJson(`/api/v1/attachments/${viewItem!.id}/documents/${documentId}`, { method: "DELETE" }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/v1/attachments"] })
    },
    onError: (e: unknown) => {
      setDocumentError(e instanceof Error ? e.message : "Delete failed")
    },
  })
  ...
  async function save() {
    ...
    try {
      const savedItem = await saveMutation.mutateAsync(body)
      if (formPdf) {
        try {
  ...
  async function printHistory() {
    ...
    const all = await fetchAllPages(async (printPage, pageSize) => {
      const rows = await apiFetchJson<HistoryEntry[]>(`/api/v1/attachments/${historyItem.id}/history?limit=${pageSize}&offset=${(printPage - 1) * pageSize}`)
  ...
  function uploadDocument(e: React.ChangeEvent<HTMLInputElement>) {
    const file = e.target.files?.[0]
    ...
    docUploadMutation.mutate({ itemId: viewItem.id, file }, {
  ...
  function deleteDocument(document: AttachmentDocument) {
    ...
    docDeleteMutation.mutate(document.id)
  }

  // ── History ───────────────────────────────────────────────────────────

  function openHistory(item: AttachmentItem) {
  ...
  const documents = (documentsQuery.data as AttachmentDocument[] | undefined) ?? []
  const documentsLoading = documentsQuery.isLoading || documentsQuery.isFetching
  const documentBusy = docUploadMutation.isPending || docDeleteMutation.isPending
  const displayDocumentError = documentError ?? (documentsQuery.isError ? "Failed to load documents" : null)
        ...
        {loading ? (
          ...
          <div className="rounded-xl border p-14 text-center space-y-3">
            ...
            <p className="text-sm text-muted-foreground">
              {searchTerm ? `No attachments matching "${searchTerm}".` : "No attachments yet."}
            </p>
            {admin && !searchTerm && (
                    ...
                    {items.map((item, i) => (
                      <tr key={item.id} className="hover:bg-muted/20 transition-colors">
                        ...
                        <td className="px-4 py-3 text-center">
                          {item.image_base64
                            ? <img src={`data:image/jpeg;base64,${item.image_base64}`} alt="att" className="size-9 rounded object-cover mx-auto" />
              ...
              {items.map(item => (
                <div key={item.id} className="rounded-lg border p-3 bg-card">
                  <div className="flex items-start gap-3">
                    {item.image_base64
                      ? <img src={`data:image/jpeg;base64,${item.image_base64}`} alt="att" className="size-12 rounded-lg object-cover shrink-0" />
          ...
          {viewItem && (
            <div className="space-y-4 mt-1">
              ...
              <div className="rounded-lg border bg-muted/20 p-3 space-y-3">
                ...
                {displayDocumentError && <p className="text-xs text-destructive">{displayDocumentError}</p>}
