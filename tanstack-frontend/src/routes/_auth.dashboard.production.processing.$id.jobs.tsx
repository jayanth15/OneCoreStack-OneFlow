...
export const Route = createFileRoute("/_auth/dashboard/production/processing/$id/jobs")({
    ...
    <>
      <PageHeader
        ...
        breadcrumbs={[
          ...
          { label: loading ? "Loading…" : order?.order_number ?? "Not found", href: dynTo(`/dashboard/production/processing/${id}`) },
          { label: "Job Cards" },
        ]}
        actions={
          <Link to={dynTo(`/dashboard/production/processing/${id}`)}
                    ...
                    <div key={process.id} className="rounded-lg border overflow-hidden">
                      ...
                      <div className="bg-muted/40 px-4 py-3 flex items-center justify-between gap-3">
                        ...
                        <Button variant="ghost" size="sm" className="h-7 text-xs shrink-0"
                          onClick={() =>
                            navigate({ href: dynTo(`/dashboard/production/processing/${order.id}/jobs/new?process=${encodeURIComponent(process.name)}`) })
                          }>
                                ...
                                <TableRow key={jc.id}>
                                  ...
                                  <TableCell className="text-right">
                                    <Button variant="ghost" size="icon" className="size-7"
                                      onClick={() => navigate({ href: dynTo(`/dashboard/production/processing/${order.id}/jobs/${jc.id}/edit`) })}
                                      title="Edit">
