...
import type {UnifiedRequest, RequestStatus, DeliverRequestPayload} from "@/lib/requests";
...
export function RequestDetailDialog({ requestId, open, onOpenChange, currentUser }: RequestDetailDialogProps) {
  ...
  const receiptsQuery = useQuery({
    queryKey: [`/api/v1/receipts/by-request/${requestId}`],
    enabled: open && requestId != null,
    staleTime: 0,
  })

  const data = requestQuery.data as UnifiedRequest | null
  const loading = requestQuery.isLoading
  const receipts = (receiptsQuery.data as Receipt[] | undefined) ?? []
  ...
  const reviewMutation = useMutation({
    ...
    }) => requestsApi.review(requestId as number, decision, options),
    onSuccess: (updated) => {
      queryClient.setQueryData([`/api/v1/requests/${requestId}`], updated)
      invalidateRequest()
    },
  })

  const acceptItemMutation = useMutation({
    mutationFn: (itemId: number) => requestsApi.acceptItem(requestId as number, itemId),
    onSuccess: (updated) => {
      queryClient.setQueryData([`/api/v1/requests/${requestId}`], updated)
      invalidateRequest()
    },
  ...
  const deliverMutation = useMutation({
    mutationFn: (payload: DeliverRequestPayload) => requestsApi.deliver(requestId as number, payload),
    onSuccess: () => {
    ...
    <Dialog open={open} onOpenChange={onOpenChange} modal={false} disablePointerDismissal>
      ...
      >
            ...
            <div className="sticky bottom-0 z-10 -mx-0 border-t bg-popover/95 backdrop-blur supports-backdrop-filter:bg-popover/80 px-6 py-3 flex items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-2 flex-wrap justify-end">
                ...
                {reviewerIsAdmin && data.request_type === "vendor_purchase" && data.status === "approved" && (
                  <Button size="sm" variant="outline" onClick={() => navigate({ href: `/dashboard/purchase-orders?from_pr=${data.id}` })}>
        ...
        <Dialog open={deliverOpen} onOpenChange={setDeliverOpen} disablePointerDismissal>
          <DialogContent className="max-w-[calc(100%-1rem)] sm:max-w-2xl p-0 gap-0 overflow-hidden">
